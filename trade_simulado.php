...
function tsSnapshot(PDO $pdo, string $runId, string $message = ''): array
{
    $data = spData(LIVE_HORIZON_MINUTES, LIVE_THRESHOLD);
    $latest = $data['latest'] ?? null;
    tradeSignalSave($data['prediction'] ?? null, $latest, [
        'source' => 'trade_simulado_snapshot',
        'run_id' => $runId,
        'minutes' => LIVE_HORIZON_MINUTES,
        'threshold' => LIVE_THRESHOLD,
...
{
    $tests = $data['tests'] ?? [];
    usort($tests, static fn(array $a, array $b): int => ((int)$a['time']) <=> ((int)$b['time']));
    ...
    foreach ($tests as $test) {
        $predicted = (int)($test['predicted'] ?? 1);
        if ($predicted === 1) {
            $skipped++;
            continue;
        }
        $btcReturn = (float)($test['return'] ?? 0);
        $tradeReturn = $predicted === 2 ? $btcReturn : -$btcReturn;
        $before = $balance;
        ...
        $orders[] = [
            'time' => (int)$test['time'],
            'end' => (int)$test['end'],
            'side' => $predicted === 2 ? 'Compra' : 'Venda',
            'predicted' => $predicted,
            'actual' => (int)($test['actual'] ?? 1),
            'btc_return' => $btcReturn,
            'trade_return' => $tradeReturn,
            'pnl' => $pnl,
            'balance_before' => $before,
            'balance_after' => $balance,
            'correct' => $predicted === (int)($test['actual'] ?? 1),
        ];
    }
    $total = count($orders);
    return [
        'initial' => LIVE_INITIAL_BALANCE,
        'balance' => $balance,
        'pnl' => $balance - LIVE_INITIAL_BALANCE,
        'return_pct' => LIVE_INITIAL_BALANCE > 0 ? (($balance / LIVE_INITIAL_BALANCE) - 1) * 100 : 0.0,
...
if (isset($_GET['run']) || $requestMethod === 'POST') {
...
<section><div class="section-head"><div><h3>Simulação com teste no passado</h3><small><?= (int)$backtest['source_tests'] ?> previsões retrospectivas da Super Previsão; sinais laterais ficam sem operação.</small></div><span class="badge">backtest <?= LIVE_HORIZON_MINUTES ?> min</span></div>
<div class="cards">
  ...
  <div class="card"><span>Operações</span><b><?= (int)$backtest['trades'] ?></b><small><?= (int)$backtest['skipped'] ?> laterais ignoradas</small></div>
</div>
<div class="table-scroll"><table><thead><tr><th>Previsão em</th><th>Fechamento</th><th>Ordem</th><th>BTC no período</th><th>Resultado trade</th><th>Lucro/prejuízo</th><th>Saldo após</th><th>Acerto</th></tr></thead><tbody>
...
<?php endforeach; ?>
<?php if (!$backtest['orders']): ?><tr><td colspan="8">Sem sinais de alta ou baixa no teste passado deste horizonte.</td></tr><?php endif; ?>
