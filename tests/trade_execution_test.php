...
$pdo->exec('CREATE TABLE trade_simulado (
 ...
 run_id TEXT,is_live INTEGER,event_type TEXT,model_version TEXT,strategy_lane TEXT,strategy_minutes INTEGER,threshold_pct REAL,
...
try {
    ...
    assertTrade(tsExecute($pdo,60,$data)['executed'],'Validated fresh quote opens both hedge lanes');
    assertTrade(!tsExecute($pdo,60,$data)['executed'],'Repeated quote cannot duplicate an order');
    $run=tsLiveRunId($pdo);$state=tsLastState($pdo,$run,'LONG_ONLY');$shortState=tsLastState($pdo,$run,'SHORT_ONLY');
    assertTrade(abs($state['qty']-.25)<1e-10 && abs($shortState['qty']-.25)<1e-10,'Each hedge lane uses 25 percent of its own equity without leverage');
    ...
    $closed=$pdo->query("SELECT * FROM trade_simulado WHERE strategy_lane='LONG_ONLY' ORDER BY id DESC LIMIT 1")->fetch();
    ...
    assertTrade((int)$pdo->query('SELECT COUNT(*) FROM trade_simulado')->fetchColumn()===4,'Failure leaves the original ledger intact');
    $pdo->exec('DROP TRIGGER fail_order');
    assertTrade(tsExecute($pdo,60,$data)['executed'],'Execution lock is released after failure');
    $state=tsLastState($pdo,$run,'LONG_ONLY');
    ...
    assertTrade((int)$pdo->query('SELECT COUNT(*) FROM trade_simulado')->fetchColumn()===$beforeCount+2,'Risk exit writes one event per open hedge lane');
